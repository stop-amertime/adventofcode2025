def find_joltage(bank):
    ...
    for pos in range(12):
            ...
            # Search space is from last start to the position where remaining digits fit
        search_space = bank[search_start : -(11-pos) or None]

            # Add the max digit in the current search space to the digits list
        digits.append(max(search_space))

            # Update search start to one position after the digit we just added
